...
- [重载运算与类型转换](#%E9%87%8D%E8%BD%BD%E8%BF%90%E7%AE%97%E4%B8%8E%E7%B1%BB%E5%9E%8B%E8%BD%AC%E6%8D%A2)
  ...
  - [下标运算分](#%E4%B8%8B%E6%A0%87%E8%BF%90%E7%AE%97%E5%88%86)
...
### 拷贝赋值运算符
- 还是一样有一个`合成拷贝赋值运算符`
- `合成拷贝赋值运算符`用来禁止该类型对象的赋值（注：这里还是不太懂）
- 行为和`拷贝构造函数`一样，但是发生在=的时候。
...
### 析构函数
- 对于给定的类，只有唯一一个析构函数。
- 成员初始化是按其在类中出现的顺序进行。而析构是`逆序销毁`，其先执行函数体的内容，再销毁成员。
...
- 由此课件会对string进行构造，然后对原来绑定的进行delete。这样是否有点浪费性能？我们可以交换他们中的`string *`指针就万事大吉了的

> 自定义swap
<div align="center"><img style="zoom:60%" src="pic/3-9.png"></div>

> 458函数应该调用swap而不是std::swap看不懂
...
- 变量表达式也有左值/右值属性，变量表达式是左值。
- 不能将一个右值引用绑定到另外一个右值引用，因为变量表达式是左值属性。
```cpp
// 1.legal
int &&r1 = 22;
// 2.illegal
...
- 形参为非const的引用
> 为什么要告诉标准库这个函数不会抛出错误
- 因为标准库默认会为处理抛出错误的可能性做一些额外的工作，所以告诉标注库不会抛出错误可以减少这些额外的工作。比如vector保证我们调用push_back的时候如果失败，vector自身不会发生改变
- 一个例子
  - 一方面，当我们用移动构造函数的时候，如果他会抛出一个异常，那么因为移动构造函数会修改右值，所以对源元素会被改变，而新构造的元素可能还没被完全初始化，在这个时候就两头空。
  - 另一方面，如果用拷贝构造函数的话，因为异常发生的时候很容易满足要求，因为这个时候修改的只是拷贝的对象，所以vector可以很容易的恢复。
...
- 如果定义两个或以上具有相同名字和相同参数列表的成员函数，在使用引用限定的时候，一个有另一个必须有。
...
- 对于一个运算符函数，或者是类的成员函数，或者至少是含有一个类类型的函数
...
## 输入和输出运算符
### 重载输出运算符
- 另输出运算不尽量减少格式化操作，这样用户在使用的时候可以更自由。比如不要在里面打印换行这些，最多用空格将这些不同的成员隔开。
...
- 如果同事还包含`==`，当且仅当`<`的定义和`==`在比较方式是一样的时候才定义`<`运算符
## 赋值运算符
- 除了拷贝赋值和移动赋值运算符，还可以定义其他运算符，`使得将别的类型作为右侧运算对象`。
- 重载的赋值运算符都需要先释放当前的内存空间，在创建一片新空间。只不过，不是拷贝赋值和移动赋值的`不用进行自身赋值的检查`
...
> 引入：funciton
- 如果我们现在想要顶一个一个寒暑表，用来存储可调用对象，当我们要用的时候从表中查找调用。我们可以这样子定义：`map<string,int(*)(int,int)> ops;`,但是这个只能insert `add函数`。
...
> 类型转换运算符可能产生意外结果
- 在早起的c++中出现的一个问题。因为istream可以转换成boll，bool是一种算术类型。所以以下这种情况会出现`<<`呈现的是移位。
```cpp
int i = 3;
cin << i;
```
...
- `如果在调用重载函数时，需要使用构造函数或者强制类型转换来改变实参的类型，这通常以为这程序设计的不足`
<div align="center"><img style="zoom:60%" src="pic/3-56.png"></div>

- 解决：显示构造

> 二义性：重载函数与用户定义的类型转换
- 即使其中一个调用需要额外的标准类型转换，编译器也会将该调用标示为错误
<div align="center"><img style="zoom:60%" src="pic/3-57.png"></div>

## 函数匹配与重载运算符
- 使用运算符重载，我们编译器不知道这个是成员的还是非成员的，所以就要综合考虑多个二义性可能。
...
- 一个派生类包含：
  - 自己定义的非静态成员
  - 继承的非静态成员
- 继承基类的部分和派生类自定义的部分不一定是连续存储的
<div align="center"><img style="zoom:60%" src="pic/3-59.png"></div>

- `可以把基类的指针和引用绑定到派生类对象中的基类部分，可以把派生类的对象当成基类对象来使用`
...
- 在派生类中的虚函数返回类型一般都和基类匹配，例外：当虚函数返回类型是类本身的指针或引用的时候，**派生类可以返回自身的引用或指针**

> final和override
- 引出override：有些时候我们想覆盖一个虚函数，但是我们不小心把形参列表写错了，这仍然是合法的，**编译器认为新定义的函数与基类中的那个是独立的**，就只是一个函数覆盖（注意：这里不是重载）。
- override：说明这个函数是来覆盖虚函数的，所以当检查到不是虚函数的时候就会报错。
...
- 类的对象不能访问
- 类的继承者可以访问
- `派生类的`成员函数 或 友元只能通过该派生类对象来访问基类的手保护成员。
...
- using声明的一般规则统一适用于重载函数的名字，基类函数的每个实例**在派生类中必须是可访问的**
...
- 当有默认实参的时候，这些参数不会被继承。而是，派生类会得到多个继承的构造函数，其中每个构造函数分别省略掉一个有默认实参的形参。
...
- 非类型参数:表示一个值,而非一个类型。
  - 当被实例化时，该参数被一个用户提供的或编译器推断出的值所代替
...
> 模板编译
- 实例化一个版本时，编译器需要知道函数模板或类模板成员函数的定义。因此**需要在其头文件中声明且定义**
...
> 在类代码内简化模板类名的使用
- **在类的作用域中**（注：也就是在类内定义），当成员函数返回模板类的类型时，可以不用模板实参。如下面可以不用`BlogPtr<T>&`。
...
- 虽然友元通常是一个类或者函数，但是这里可以用一个内置类型来实例化Bar，这种与内置类型的友好关系是允许的
...
### 效率与灵活性
- 这节不容易总结，重要。建议多看几遍
- 介绍了shared_ptr和unique_ptr之间的明显不同
  - 一种是动态绑定删除器
  - 一种是编译时绑定删除器

## 模板实参推断
### 类型转换与模板类型参数
- 如果一个**函数形参**的类型使用了**模板类型参数**，那么它采用特殊的初始化规则
- 当我们给一个模板函数传入实参进行调用的时候，要让这些实参进行类型转换的情况只有几种。通常编译器**不是对实参进行类型转换，而是生成一个模板实例**。
