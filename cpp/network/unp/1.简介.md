...
## read和write
- **字节流上调用read和write**输入或输出的字节数可能比请求的数量少，然而这不是出错的状态。可能原因在于**内核中的用于套接字的缓冲区已经到达了极限**（read可能是真的遇到了EOF），这个时候需要再次调用read和write函数，以输入或输出剩余的字节。
...
<div style="zoom: 80%" align="center"> <img src="./pic/3-3.png"></div>
