...
- [迭代器](#迭代器)
  ...
  - [要点总结](#要点总结-1)
- [Chain of Resposibility职责链](#chain-of-resposibility职责链)
  - [动机](#动机-1)
  - [代码](#代码-2)
  - [模式定义](#模式定义-2)
  - [结构(Structure)](#结构structure-2)
  - [要点总结](#要点总结-2)
- [总结](#总结)
# "数据结构"模式
...
class Composite:public Component{
    List<Component*> m_leaf;
public:
    Composite* getComposite() override {return this;}
    void add(Componet* child);
    void remove(int idx);
...
通过RTTI，程序能够使用基类的指针或引用来检查这些指针或引用所指的对象的实际派生类型。
  - C++内建的 RTTI 中的运算符：typeid 和 dynamic_cast。
  - C#、Delphi等的RTTI运算符：is 和 as
## 要点总结
- Composite模式采用树形结构来实现普遍存在的对象容器，从而将“**一对多**”的关系转化为“**一对一**”的关系 **（注：意思是如果不用组合模式，那么如果Client遇到非叶子节点就需要使用一对多，而这里通过多态的递归调用后，就变成了一对一的关系）**，使得客户代码可以一致地(复用)处理对象和对象容器，无需关心处理的是单个的对象，还是组合的对象容器。
